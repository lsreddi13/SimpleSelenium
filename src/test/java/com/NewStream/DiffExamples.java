...
public class DiffExamples {
	public static void main(String[] args) {
		...
		// 2
		// List<String> collect = linksList.stream().filter(ele ->
		// !ele.getText().equals("")).map(ele -> ele.getText()).
		// collect(Collectors.toList());
		//
		// collect.forEach(ele -> System.out.println(ele));

		// 3
		// String firstEle = linksList.stream().filter(ele ->
		// !ele.getText().equals("")).findFirst().get().getText();
		// System.out.println(firstEle);

		// 4
		// String text = linksList.stream().filter(ele ->
	...
	public static String switchToWindow(WebDriver driver, String title) {
		return driver.getWindowHandles().stream().map(windowHandle -> driver.switchTo().window(windowHandle).getTitle())
